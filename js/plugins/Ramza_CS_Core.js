...
Ramza.CS = Ramza.CS || {};
Ramza.CSCore.version = 1.10
var Imported = Imported || {};


//=============================================================================
 /*:
 * @plugindesc v1.10 Core plugin required for the Ramza_CraftingSystem plugins to function.
 * @author Ramza
 * @target MZ
 ...
 * ============================================================================
 * Version 1.10:
 * -Added a new additive trait: SET ICON: X. The result item will have its 
 *  icon set to X if an ingredient has this tag. If multiple ingredients have 
 *  this tag the last one in the recipe will be the one that is used.
 * -Added RESET ICON additive trait, which overrides the above tag and resets
 *  the result items icon back to the default.
 * -Added additive traits for block chance, block value, can/cannot block, 
 *  parry chance, and expertise. These require the block chance/shield 
 *  block/parry chance plugins to function.
 * Version 1.09:
 * -Added a new additive trait MODIFYPARAM: param value
 * -This additive trait works similar to the changeparam trait, but only 
 *  applies the param change to a result item that already had a value for the
 *  given param.
 * Version 1.08:
 * -Added a new additive trait, COST: value. This trait passes a flat 
 *  value onto the gold value of the result item. 
 * -Ramza.CS.Core.version will now report the correct version of the CS Core.
 * Version 1.07:
 * -Corrected an issue where the Mp Recovery+ additive trait was parsed 
 *  incorrectly, causing any result items that gained Mp recovery from an 
 *  additive trait to restore NAN% MP + NaN.
 * Version 1.06:
 * -Corrected an issue where, upon crafting level up, a level up message would 
 *  always be drawn in the result window, causing an 'undefined' string to be 
 *  drawn if the 'Use a Level Up Message?' plugin parameter was set to false.
 * Version 1.05:
 * -(revision a) Corrected an issue where ingredients that we not consumed 
 *  during crafting were not providing additive traits to the result item.
 * -Corrected an issue where completing a recipe that did not consume any 
 *  ingredients would crash the project.
 * Version 1.04:
 * -Minor adjustment to the way that the CraftingLevels are defined on 
 *  database load.
 * Version 1.03:
 * -Corrected a long-standing bug, where when searching for a partial recipe 
 *  match, while using YEP_ItemCore, the plugin would cause a crash attempting 
 *  to check if a provided ingredient was an independent item or not.
 * Version 1.02:
 * -Corrected an issue where ADD SKILL: X, and ADD STYPE: X additive traits 
 *  notetags were being parsed incorrectly.
 * Version 1.01:
 * -Version update to allow for more item categories in the item category 
 *  window.
 * Version 1.00:
...
DataManager.updateAdditiveTraits = function(item, string){
	...
	var note31 = /(?:COST:[ ]*([-+]?\d+)([%]?))/i
	var note33 = /(?:SET ICON:[ ]+(\d+))/i
	var note34 = /(?:BLOCK CHANCE:[ ]+([-+]?\d+))/i
	var note35 = /(?:BLOCK VALUE:[ ]+([-+]?\d+))/i
	var note36 = /(?:(CAN|CANNOT)+[ ]+BLOCK)/i
	var note37 = /(?:PARRY CHANCE:[ ]+([-+]?\d+))/i
	var note38 = /(?:EXPERTISE:[ ]+([-+]?\d+))/i
	var note39 = /(?:BLOCK PERCENT:[ ]+([-+]?\d+))/i
	var note40 = /(?:RESET ICON)/i
	
	if (string.match(note1) || string.match(note2) || string.match(note32)){
		...
		if (paramId != -1 && string.match(note32)) item.CS._addParamMod.data[paramId] = parseInt(RegExp.$2)
	...
	} else if (string.match(note31)){
		item.CS._cost.data[0] = Number(RegExp.$1)
		item.CS._cost.data[1] = false
		//item.CS._cost.data[1] = (RegExp.$2) ? true : false
	} else if (string.match(note33)){
		item.CS._icon = {dataType: 12, data: Number(RegExp.$1)}
	} else if (string.match(note34)){
		item.CS._blockRate = {dataType: 5, data: Number(RegExp.$1)}
	} else if (string.match(note35)){
		item.CS._blockValue = {dataType: 5, data: Number(RegExp.$1)}

	} else if (string.match(note36)){
		switch (String(RegExp.$1).toLowerCase()){
			case "can":
			item.CS._canBlock = {dataType: 6, data: true}
			break;
			case "cannot":
			item.CS._canBlock = {dataType: 6, data: false}
			break;
			default:
			item.CS._canBlock = {dataType: 6, data: false}
			break;
		}
	} else if (string.match(note37)){
		item.CS._parryRate = {dataType: 5, data: Number(RegExp.$1)}
	} else if (string.match(note38)){
		item.CS._expertise = {dataType: 5, data: Number(RegExp.$1)}
	} else if (string.match(note39)){
		item.CS._blockPercent = {dataType: 5, data: Number(RegExp.$1)}
	} else if (string.match(note40)){
		item.CS._resetIcon = {dataType: 6, data: true}
	}
};
...
Ramza.CS.getDefaultAdditiveTraits = function(){
	...
	additiveTraits._addParamMod = {dataType: 1}
	additiveTraits._addParamMod.data = this.returnDefaultDatasetForType(additiveTraits._addParamMod.dataType)
	...
	additiveTraits._cost = {dataType: 11}
	additiveTraits._cost.data = this.returnDefaultDatasetForType(additiveTraits._cost.dataType)
	additiveTraits._icon = {dataType: 12}
	additiveTraits._icon.data = this.returnDefaultDatasetForType(additiveTraits._icon.dataType)
	additiveTraits._resetIcon = {dataType: 6}
	additiveTraits._resetIcon.data = this.returnDefaultDatasetForType(additiveTraits._resetIcon.dataType)
	additiveTraits._blockRate = {dataType: 5}
	additiveTraits._blockRate.data = this.returnDefaultDatasetForType(additiveTraits._blockRate.dataType)
	additiveTraits._blockValue = {dataType: 5}
	additiveTraits._blockValue.data = this.returnDefaultDatasetForType(additiveTraits._blockValue.dataType)
	additiveTraits._canBlock = {dataType: 6}
	additiveTraits._canBlock.data = this.returnDefaultDatasetForType(additiveTraits._canBlock.dataType)
	additiveTraits._parryRate = {dataType: 5}
	additiveTraits._parryRate.data = this.returnDefaultDatasetForType(additiveTraits._parryRate.dataType)
	additiveTraits._blockPercent = {dataType: 5}
	additiveTraits._blockPercent.data = this.returnDefaultDatasetForType(additiveTraits._blockPercent.dataType)
	additiveTraits._expertise = {dataType: 5}
	additiveTraits._expertise.data = this.returnDefaultDatasetForType(additiveTraits._expertise.dataType)
...
Ramza.CS.returnDefaultDatasetForType = function(dataTypeId) {
	...
	switch (dataTypeId) {
		...
		break
		case 11:
		//cost array (value, whether or not it's a float)
		return [0, false]
		break
		case 12:
		//change icon (-1 resets it)
		return -1
		break
		default:
...
Ramza.CS.removeOldAdditiveEffects = function(item){
	...
	item.iconIndex = item.baseItemIconIndex
...
Ramza.CS.createDonorItem = function(ingredientlist){
	...
	//if (!ingredientlist) return donorItem
...
Ramza.CS.removeOldAdditiveTraits = function(item){
	...
	for (i = 0; i < 8; i++){
		if (item.params[i]) item.params[i] -= item._oldAdditives._addParamMod.data[i]
	}
...
Ramza.CS.addTrait = function(result, traitType){
	switch (traitType){
		case "_cost":
		if (result.CSU._cost.data[1]){
			result.price = Math.round(result.price * result.CSU._cost.data[0] / 100)
		} else {
			result.price += result.CSU._cost.data[0]
		}
		break;
		...
		case "_addParamMod":
		for (let i = 0; i < 8; i++){
			if (result.params[i]) result.params[i] += result.CSU._addParamMod.data[i]
		}
		break;
		case "_addParamRate":
		for (let i = 0; i < 8; i++){
		...
		break;
		case "_resetIcon":
		if (result.CSU._resetIcon == true){
			result.iconIndex = result.baseItemIconIndex
			result.CSU._icon.data = -1
		}
		break;
		case "_icon":
		if (result.CSU._icon.data != -1 && !result.CSU._resetIcon.data){
			result.iconIndex = result.CSU._icon.data
		} else {
			result.iconIndex = result.baseItemIconIndex
		}
		break;
		...
		break;
		case "_blockRate":
		var change = Number((result.CSU._blockRate.data / 100).toFixed(2))
		var traitAdd = {code: 22, dataId: 10, value: change}
		result.CSU.addedTraits.push(result.traits.length)
		result.traits.push(traitAdd)
		break;
		case "_blockValue":
		result.blockValue = result.blockValue || 0
		result.blockValue = Math.max(result.blockValue + result.CSU._blockValue.data, 0)
		break;
		case "_blockPercent":
		result.blockPercent = result.blockPercent || 0 
		result.blockPercent = Math.min((result.blockPercent + result.CSU._blockPercent.data), 100) 
		break;
		case "_canBlock":
		//this only works for RMMZ Shield Block, RMMV requires a block enabling state
		result._enablesBlock = result.CSU._canBlock.data
		break;
		case "_parryRate":
		result._parryChance += (result.CSU._parryRate.data / 100).toFixed(2)
		break;
		case "_expertise":
		result._expertise += (result.CSU._expertise.data / 100).toFixed(2)
		break;
		case "addedTraits":
		break;
		case "removedTraits":
...
Ramza.CS.addEffect = function (result, traitType){
	...
	switch (traitType){
		...
		if (result.CSU._mpRecovery.data != null){
			var effectAdd = {code: 12, dataId: 0, value1: result.CSU._mpRecovery.data[0], value2: result.CSU._mpRecovery.data[1]}
...
Ramza.CS.pushOldAdditiveEffects = function(item){
	...
	if (item.CSU && item.CS && item.CS._addParamChange != undefined){
		for (const property in item.CS){
			if (item.CS[property] == 42) console.log(property)
			Ramza.CS.combineAdditiveTraits(item.CS[property], item.CSU[property])
		}
		item.CSU = {}
	}
...
Ramza.CS.combineAdditiveTraits = function(target, ingredient) {
	...
	switch (ingredient.dataType){
		...
		if (target.data == null) {
			...
			//tp recovery, block rate, block value, parry rate, expertise
			target.data += ingredient.data
		}
		break
		case 6:
		...
		case 11:
		//(cost change)
		target.data[0] += ingredient.data[0]
		target.data[1] = (target.data[1] || ingredient.data[1])
		break;
		case 12:
		if (ingredient.data != -1) target.data = ingredient.data  
		break;
		default:
					...
					for (u = 0; u < inputArray.length; u++){
						if(!Imported.YEP_ItemCore){
							...
							if (inputArray[u].item == recipe.itemList[k] && !inputArray[u].used){
								...
								} else if (inputArray[u].item.etypeId && inputArray[u].item.etypeId == 1 && inputArray[u].item.nonIndependent){
									//unique weapon found
...
Window_IngredientCategory.prototype.maxCols = function() {
	let cols = 0
	if (Ramza.CSParams.CategoryCommands.Item) cols += 1
	if (Ramza.CSParams.CategoryCommands.Weapon) cols += 1
	if (Ramza.CSParams.CategoryCommands.Armor) cols += 1
	if (Ramza.CSParams.CategoryCommands.UseClear) cols += 1
	if (Ramza.CSParams.CategoryCommands.Hidden1) cols += 1
	if (Ramza.CSParams.CategoryCommands.Hidden2) cols += 1
	if (Ramza.CSParams.CategoryCommands.KeyItem) cols += 1
    return cols;
};

Window_IngredientCategory.prototype.maxRows = function() {
    return 1;
};
...
Window_IngredientCategory.prototype.makeCommandList = function() {
    if (Ramza.CSParams.CategoryCommands.Item) this.addCommand(TextManager.item,    'item');
    if (Ramza.CSParams.CategoryCommands.KeyItem) this.addCommand(Ramza.CSParams.CategoryCommands.KeyItemText,    'KeyItem');
    if (Ramza.CSParams.CategoryCommands.Hidden1) this.addCommand(Ramza.CSParams.CategoryCommands.Hidden1Text,    'hidden1');
    if (Ramza.CSParams.CategoryCommands.Hidden2) this.addCommand(Ramza.CSParams.CategoryCommands.Hidden2Text,    'hidden2');
    if (Ramza.CSParams.CategoryCommands.Weapon) this.addCommand(TextManager.weapon,  'weapon');
    if (Ramza.CSParams.CategoryCommands.Armor) this.addCommand(TextManager.armor,   'armor');
    if (Ramza.CSParams.CategoryCommands.UseClear) this.addCommand(Ramza.CSParams.ClearText,   'clear');
};
...
Window_IngredientList.prototype.includes = function(item) {
	if (Ramza.CSParams.UseCategoryWindow){
		...
		case 'hidden1':
			return DataManager.isItem(item) && item.itypeId === 3 && this.correctCategory(item);
		case 'hidden2':
			return DataManager.isItem(item) && item.itypeId === 4 && this.correctCategory(item);
		case 'weapon':
...
Window_CraftingResult.prototype.update = function(){
	...
	if ($gameTemp.levelUp){
		...
		if (Ramza.CSParams.showLevelUpMessage) this.drawText(SceneManager._scene.getLevelUpString(), 0, 120, this.width - 32)
...
Window_CraftingResult.prototype.removeAllIngredients = function(recipe){
	...
	this._lostValues = []
		for (i = 0; i < this._ingList.length; i++){
			...
			} else if (!this._ingList[i]._consumedOnSuccess && !inrecipe){
				$gameParty.gainItem(this._ingList[i], -(0))
				this._lostItems.push(this._ingList[i])
				this._lostValues.push(this._ingCount[i])
...
Window_CraftingResult.prototype.removeIngredients = function(recipe){
	if (!Ramza.CSParams.KeepExcessMaterials){
		if (recipe.BaseItem != "Category"){
			...
			if (!recipe.BaseItem._consumedOnSuccess) $gameParty.loseIngredientItem(adjList[0], -(0))
			...
			if (!this._ingList[ingIndex]._consumedOnSuccess) $gameParty.loseIngredientItem(this._ingList[ingIndex], -(0))
			this._ingList.splice(ingIndex, 1)
		}
		if (recipe.SecondItem != "Category"){
			...
			if (!recipe.SecondItem._consumedOnSuccess) $gameParty.loseIngredientItem(adjList[0], -(0))
			this._ingList.splice(ingIndex, 1)
		} else {
		...
		if (recipe.ThirdItem != "Category"){
			...
			if (recipe.ThirdItem != null && !recipe.ThirdItem._consumedOnSuccess) $gameParty.loseIngredientItem(adjList[0], -(0))
			this._ingList.splice(ingIndex, 1)
		} else {
		...
		if (recipe.FourthItem != "Category"){
			...
			if (recipe.FourthItem != null && !recipe.FourthItem._consumedOnSuccess) $gameParty.loseIngredientItem(adjList[0], -(0))
			this._ingList.splice(ingIndex, 1)
		} else {
